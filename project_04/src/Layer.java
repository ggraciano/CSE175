...
public class Layer {
	...
	public void computeOutputDelta() {
		...
		// diff calculates the difference between the targ, the target
		// vector and act, the activation levels of units, which is the
		// the error.
		diff = targ.difference(act);
		
		// deriv calculates the derivative of the net, that is, the
		// activation function.
		deriv = net.derivative(min, max);
		
		// Create a new instance of delta, with dimension n.
		delta = new Vector(n, 0.0);
		
		// The gradient values that make up delta, are given by
		// multiplying diff and deriv at all instances.
		for (int i = 0; i < n; i++) {
			delta.set(i, diff.get(i) * deriv.get(i));
		}
		
		// The sum, delta, is then given by summing up the gradient
		// values that make up delta.
		delta.sumOfElements();
	...
	public void computeHiddenDelta() {
		...
		// Create a new instance of diff, the error, with dimension n,
		// since there are already input values.
		diff = new Vector(n, 0.0);
		...
		// Create a map of the downstream of the units in the network,
		// that is, all units whose inputs include the output of a
		// given unit.
		for (Projection output : outputs) {
			// Using the outputs, determine the error, diff, for the
			// output units, stored in a matrix, and then back propagate
			// this error by layer through the network, thus changing
			// the weights at each layer.
			Matrix temp = (output.W.transpose());
			diff = diff.sum(temp.product(output.output.delta));
		}
