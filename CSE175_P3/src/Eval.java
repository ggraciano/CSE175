...
public class Eval {
	...
	static double value_roll_hand(State s, int depth) {
		...
		// The expected utility value of the given state is calculated
		// by considering the utilities of all of the possible next
		// states, which is done by iterating over all possible roll
		// outcomes.
		for (DieFace up1 : DieFace.values()) {
			// Dice cannot be invalid;.
			...
			for (DieFace up2 : DieFace.values()) {
				...
				for (DieFace up3 : DieFace.values()) {
					...
					// Calculate the probability of rolling the
					// three dice.
					double pThree = s.rollProb(up1, up2, up3);
					// Roll the three ice.
					s.roll(up1, up2, up3);
					// Sum the calculated utility of the rolled state
					// with the previous utilities.
					val += pThree * value_rolled_hand(s, depth);
				}
			}
		}
	...
	static public double heuristic(State s) {
		...
		// Number of brains eaten by each player
		// 	- Player with more brains +1
		// Current player's turn
		// 	- Current player +1
		// Number of brains collected by the current player
		// 	- More is better
		// Number of blasts collected by the current player
		// 	- Less is better
		// Current player's decision to roll again or end turn
		// 	- Various factors are taken into account to make their choice
		// Dice remaining in the cup
		// 	- Number and color of the dice affect the player's decision
		
		// If the computer has eaten more brains and currently holds more
		// brains than the user, then the computer has won.
		if (s.comp_brains_eaten + s.brains_collected >= State.brains_to_win) {
			return (State.win_payoff);
		}
		
		// If the current player is the computer, then the computer is in
		// the lead, that is, relative to the amount of players and vice
		// versa.
		if (s.current_player == Turn.computer) {
			value += (Turn.values().length - 1) * State.win_payoff;
		} else {
			value -= (Turn.values().length - 1) * State.win_payoff;
		}
		
		// comp_brains_eaten_diff, here, evaluates the difference between
		// the number of brains eaten and currently collected by the
		// computer and the brains eaten by the user. As such, if
		// comp_brains_eaten_diff is negative, then the user has eaten more
		// brains than the computer, and thus, is in the lead, and vice
		// versa.
		double comp_brains_eaten_diff = s.comp_brains_eaten + s.brains_collected
				- s.user_brains_eaten;
		
		// To translate this feature into a numeric expected payoff,
		// calculate the progress of the game with respect to the computer,
		// that is, the ratio of the player currently in the lead,
		// comp_brains_eaten_diff,to the number of brains necessary to win
		// the game, and multiply it by the win payoff.
		value += (comp_brains_eaten_diff / State.brains_to_win) * State.win_payoff;
		
		// s.blasts_collected cannot be greater than or equal to
		// State.num_terminal_blasts as the current player's turn is over
		// when s.blasts_collected is equal to State.num_terminal_blasts.
		// Thus, from the computer's perspective, it should always gamble
		// relative to the amount of blasts it has collected, that is, risk
		// more when it has collected less blasts and vice versa. In this
		// case, since either player can only collect two blasts before
		// their turn is over, the computer should bet higher when they
		// have only collected one blast and vice versa. Moreover, these
		// rules should only apply when the computer has already collected
		// brains as it has more to lose. However, if the computer has not
		// collected any blasts or brains, then it has nothing to lose
		// and thus, should bet the maximum, that is, the win payoff.
		if ((s.blasts_collected == 1 || s.blasts_collected == 2) && s.comp_brains_eaten > 0) {
			value += ((State.num_terminal_blasts - s.blasts_collected)
					/ State.num_terminal_blasts) * State.win_payoff;
		} else {
			value += State.win_payoff;
		}
		
		// Return the values relative to the users perspective, thats is,
		// multiply value by negative one.
		value *= -1;
		
		// If value is greater than or equal to one, then the user is in
		// the lead, else the computer is in the lead.
		if (value >= 0) {
			return (State.win_payoff);
		} else {
			return (-State.win_payoff);
